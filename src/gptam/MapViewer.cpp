...
{
  mse3ViewerFromWorld = 
    SE3<>::exp(cv::Vec<float, 6>(0,0,2,0,0,0)) * SE3<>::exp(cv::Vec<float, 6>(0,0,0,0.8 * M_PI,0,0));
}
...
void MapViewer::DrawMap(SE3<> se3CamFromWorld)
{
  ...
  {
    pair<cv::Vec<float, 6>, cv::Vec<float, 6> > pv6 = mGLWindow.GetMousePoseUpdate();
    SE3<> se3CamFromMC;
    se3CamFromMC.get_translation() = mse3ViewerFromWorld * mv3BaryCenter;
    mse3ViewerFromWorld = SE3<>::exp(pv6.first) * 
			  se3CamFromMC * SE3<>::exp(pv6.second) * 
			  se3CamFromMC.inverse() * 
			  mse3ViewerFromWorld;
  }
...
void MapViewer::DrawCamera(SE3<> se3CfromW, bool bSmall)
{
  ...
  if(!bSmall)
  {
    ...
    glColor3f(0.5,0.5,0.5);
    SetupModelView();
    cv::Vec2f v2CamPosXY( se3CfromW.inverse().get_translation()[0], 
			  se3CfromW.inverse().get_translation()[1] );
    glBegin(GL_LINES);
    glColor3f(1,1,1);
