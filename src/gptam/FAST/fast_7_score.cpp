...
inline int fast_corner_score_7(const uchar* cache_0, const int offset[], int b)
{	
	b++;
...
void fast_corner_score_7(const cv::Mat_<uchar> &i, const vector<cv::Point2i> &corners, int b, vector<int> &scores)
{
	...
	for(unsigned int n=0; n < corners.size(); n++)   
	  scores[n] = fast_corner_score_7(i.ptr<uchar>(corners[n].y, corners[n].x), pixel, b);
