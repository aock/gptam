...
#include "gptam/Persistence/GUI.h"
#include "gptam/Persistence/GUI_impl.h"
...
namespace Persistence
{
	...
	GUI_language* get_new_lang(); // need to look into thisssss...
	void remove_lang(GUI_language* l); // same here...
  ...
  // unregister a command regardless of GUI owner
  void GUI_impl::UnRegisterCommand(string sCommandName)
  {
    mmCallBackMap.erase(sCommandName);
  };

  // unregister all commands from the same GUI object
  ...
  // unrtegister command from a specific GUI (thisptr)
  ...
  // Ok, this is how we register a "GUI command" by entering the callback 
  void GUI_impl::RegisterCommand(string sCommandName, GUICallbackProc callback, void* thisptr)
  {
    // check if command name is already registerd in the list (builtins)
    ...
    // ok, command name is not taken. Create callback info struct with the callback pointer and the pointer to the GUI (thisptr) that
    // registers the command
    CallbackInfoStruct s;
    s.cbp = callback;
    s.thisptr = thisptr;

    // check callback map (a hash table-std::map- <"commandname" , "callbackpointer"> if callback pointer is used  
	...
	bAlreadyThere=true;
      ...
      // ok, callback not in the map. Insert it.
    if(!bAlreadyThere) cbv->push_back(s);
  };


  // Checks (by name) and executes callbacks returns true; return false otherwise...
  ...
  // ************* This is important! Loading configuration file!!!! **************************  
  ...
  // This concerns a bit of elaborate syntax inherited from GVars (and won;t be making much use of, but may consider it in the future!)
  // Brilliant work!
  void GUI_impl::ParseLine(string s, bool bSilentFailure )
  {
	...
	{   // Brace Pair found!!
	  ...
	  if(nLength>2)
	    if((sVarName[0]=='{') && (sVarName[nLength-1]=='}')) bIsDoubleQuoted = true;
	
	  if(!bIsDoubleQuoted) {
	    
	      //vector<string> vsExpandedList = ChopAndUnquoteString(mpGV2->StringValue(sVarName, true));
	      vector<string> vsExpandedList = ChopAndUnquoteString(PV3::get_var(sVarName));
	  ...
	  if(!bIsDoubleQuoted)
	    {
	      ...
	      string sExpanded = PV3::get_var(sVarName);
  ...
  void builtin_set(void* /*ptr*/, string /*sCommand*/, string sParams)
  {
	...
	void builtin_gvarlist(void* /*ptr*/, string sCommand, string sParams)
	{
		...
		if(error)
			...
			PV3::print_var_list(cout, pattern, print_all);
	}


  void builtin_printvar(void* /*ptr*/, string /*sCommand*/, string sParams)
  {
    cout << sParams << "=" << PV3::get_var(sParams) << endl;;
  }
