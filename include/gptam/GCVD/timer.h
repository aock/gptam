// George: Lifted this file verbatim to save (and log) time. 
// We just need high accurate timing for mappoints.
...
class SimpleTimer
{
    public:
        ...
        /// Destructor. Deletes the internal timer
        ~SimpleTimer()
        {
            delete internal_Timer;
        }
        ...
        void click()
        {
                ...
                {
                    timing_started=true;
                    time_at_start_of_timing=internal_Timer->get_time();
                }
                else if (timing_started)
                {
                    increment=internal_Timer->get_time()-time_at_start_of_timing;
